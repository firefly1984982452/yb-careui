<template>
  <div class="page">
    ...
    <van-form @submit="onSubmit">
      <div style="margin: .08rem 0 0 0;">
        <div class="leave-reason">
          ...
          <div class="radio">
            ...
            <div v-for="(reason,reasonIndex) in reasons" @click="chooseLeave(reason,reasonIndex)" :key="reasonIndex" :class="iscur === reasonIndex ? 'active' : ''">
              {{ reason }}
            </div>
          </div>
        </div>
        <van-field v-model="leaveMessage" rows="2" maxlength="200" show-word-limit autosize label="" type="textarea" placeholder="请输入请假详细说明" />
      </div>
      <div style="margin: .08rem 0 0 0;">
        <van-field class="date" readonly :value="leaveData" label="离院时间" placeholder="点击选择离院时间" :rules="[{ required: true, message: '请选择离院时间' }]" @click="showPickerleaveData = true" />
        <van-popup v-model="showPickerleaveData" position="bottom">
          <van-datetime-picker v-model="leaveDataTime" type="datetime" :min-date="minDate" :max-date="maxDate" :formatter="formatter" @confirm="confirmleaveData()" @cancel="showPickerleaveData = false" />
        </van-popup>
      ...
      <!-- <div style="margin: .08rem 0 0 0;">
        <van-field class="date" readonly :value="backData" label="回院时间" placeholder="点击选择回院时间" @click="showPickerBackData = true" :rules="[{ required: true, message: '请选择回院时间' }]" />
        <van-popup v-model="showPickerBackData" position="bottom">
          <van-datetime-picker v-model="backDataTime" type="datetime" :min-date="minDate" :max-date="maxDate" title="选择完整回院时间" :formatter="formatter" @confirm="confirmBackData()" @cancel="showPickerBackData = false" />
        </van-popup>
        <van-field class="date" name="switch" label="是否就餐">
          <template #input>
...
export default {
  ...
  methods: {
    ...
    confirmleaveData() {
      this.leaveData = this.$moment(this.leaveDataTime).format(
        "YYYY-MM-DD HH:mm:ss"
      );
      this.showPickerleaveData = false;
    },
    // confirmBackData() {
    //   this.backData = this.$moment(this.backDataTime).format(
    //     "YYYY-MM-DD HH:mm:ss"
    //   );
    ...
    onSubmit(values) {
      ...
      axios
        ...
        .then((res) => {
          if (res.code == 200) {
            axios
              .fetch("elderServer", "/elderLeave/update", data)
              .then((res) => {
                if (res.code == 200) {
                  this.$toast("请假登记成功！");
                  this.$router.go(-1);
